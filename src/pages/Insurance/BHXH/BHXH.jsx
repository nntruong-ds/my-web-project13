...
import {getBHXH, exportBHXH, updateBHXH, createBHXH} from "../../../api/bhxh";
...
export default function BHXH() {
    ...
    const fetchBHXH = async () => {
        try {
            const data = await getBHXH({
    ...
    const handleExport = async () => {
        try {
            ...
            const blob = await exportBHXH(filters);

            // Tạo link download
            const url = window.URL.createObjectURL(blob);
            const a = document.createElement("a");
            a.href = url;
            a.download = "danh_sach_bhxh.xlsx";
    ...
    const handleUpdate = async () => {
        try {
            const payload = {
                ...
                so_so_bhxh: updateData.so_so_bhxh,
                thang_nam: updateData.thang_nam?.slice(0, 10),
                thang: updateData.thang ? Number(updateData.thang) : null,
                trang_thai: updateData.trang_thai,
            };
            await updateBHXH(payload);
            alert("Cập nhật BHXH thành công");
            setShowUpdate(false);
            await fetchBHXH();
        } catch (err) {
            console.error(err);
            alert(err.response?.data?.message || "Cập nhật BHXH thất bại");
        }
    };
    return (
            <div className={styles.container}>
                <div className={styles.content}>
                    <div className={styles.header}>
                        ...
                        THEO DÕI ĐÓNG BHXH THÁNG {searchMonth}-{searchYear}
                    ...
                    <div className={styles.func}>
                        <div className={styles.search}>
                            ...
                            <div className={styles.row}>
                                ...
                                />
                                <button onClick={fetchBHXH}>Tìm</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className={styles.overlay}>
                        <div className={styles.updateBox}>
                            ...
                            <input
                                ...
                                value={updateData.so_so_bhxh}
                                onChange={e => setUpdateData({...updateData, so_so_bhxh: e.target.value})}
                            />

                            <input
                                type="date"
                                value={updateData.thang_nam}
                                onChange={e => setUpdateData({...updateData, thang_nam: e.target.value})}
                            />
                ...
                {showCreate && (
                    <div className={styles.overlay}>
                        <div className={styles.updateBox}>
                            ...
                            <input
                                placeholder="Số sổ BHXH"
                                value={createData.so_so_bhxh}
                                onChange={e =>
                                    setCreateData({...createData, so_so_bhxh: e.target.value})
                                }
                            />
